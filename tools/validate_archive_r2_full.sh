...
RATES="2 4 8 16 32"
DIR="./results/RTX-8000-archive-r2-full"
...
cleanup
# Pre-flight: refuse if another vllm/bench is actively running (would corrupt measurements).
if pgrep -f "VLLM::EngineCore|bench serve|vllm.entrypoints" > /dev/null; then
    echo "FATAL: vllm/bench processes survived cleanup. Another script active?"
    ps aux | grep -E "vllm|bench serve" | grep -v grep
    exit 1
fi
env \
