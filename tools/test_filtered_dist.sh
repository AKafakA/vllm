#!/bin/bash
# Distribution oracle on FILTERED profile.
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
source ~/Code/llm/vllm-emulator/.venv/bin/activate
cd ~/Code/llm/vllm-emulator

LOG=/tmp/vllm_filtered_dist.log
echo "=== Filtered+distribution test at $(date) ===" > "$LOG"

MODEL="Qwen/Qwen3-8B"
PORT=8100
NP=200
PROFILE="./results/RTX-8000-adaptive/profiles/serving-Qwen3-8B-adaptive-filtered.json"
RESULT_DIR="./results/RTX-8000-adaptive/filtered-dist-sweep"
mkdir -p "$RESULT_DIR"
...
run_rate() {
    ...
    # Reuse real results from previous sweep (same profile, same setup)
    REAL_SRC="./results/RTX-8000-adaptive/filtered-sweep/${TAG}_real.json"
    if [ -f "$REAL_SRC" ]; then
        cp "$REAL_SRC" "$RESULT_DIR/${TAG}_real.json"
    fi

    # EMU with distribution oracle
    cleanup
...
echo "" >> "$LOG"
echo "=== SUMMARY: filtered + distribution ===" >> "$LOG"
python3 -c "
import json
print(f'{\"Rate\":>5} {\"TPOT%\":>8} {\"TTFT%\":>8} {\"P99TPOT%\":>9} {\"Status\":>8}')
for rate in [1, 2, 4, 8]:
    try:
        ...
        print(f'{rate:>5} {tpot:>+7.1f}% {ttft:>+7.1f}% {p99:>+8.1f}% {status:>8}')
