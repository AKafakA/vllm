#!/bin/bash
# Phase 2 — v5-arrival-sample variant: per-arrival random draw from raw
# IPC overhead distribution. Full 5-rate × 2000p validation.
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
source ~/Code/llm/vllm-emulator/.venv/bin/activate
cd ~/Code/llm/vllm-emulator

MASTER_LOG="/tmp/vllm_apr20_phase2.log"
echo "=== phase2 v5-arrival-sample start $(date) ===" > "$MASTER_LOG"
touch /tmp/vllm_apr20_phase2.started

# Wait for Phase 1 to finish (gate).
for i in $(seq 1 180); do
    [ -f /tmp/vllm_apr20_phase1.done ] && break
    sleep 30
done
if [ ! -f /tmp/vllm_apr20_phase1.done ]; then
    echo "  Phase 1 did not complete in time; aborting" >> "$MASTER_LOG"
    exit 1
fi
...
preflight() {
    if pgrep -f "VLLM::EngineCore|bench serve|vllm.entrypoints" > /dev/null; then
        echo "FATAL: procs survived cleanup" >> "$MASTER_LOG"
        exit 1
    fi
}
...
echo "" >> "$MASTER_LOG"
echo "=== phase2 DONE $(date) ===" >> "$MASTER_LOG"
touch /tmp/vllm_apr20_phase2.done
