#!/bin/bash
# Test distribution mode across rates 1, 2, 4, 8 to understand rate dependency.
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
source ~/Code/llm/vllm-emulator/.venv/bin/activate
cd ~/Code/llm/vllm-emulator

LOG=/tmp/vllm_rate_sweep.log
echo "=== Rate sweep test at $(date) ===" > "$LOG"

MODEL="Qwen/Qwen3-8B"
PORT=8100
NP=200
PROFILE="./results/RTX-8000-dense/profiles/serving-Qwen3-8B-dense.json"
RESULT_DIR="./results/RTX-8000-dense/rate-sweep"
mkdir -p "$RESULT_DIR"
...
run_rate() {
    ...
    echo "" >> "$LOG"
    echo "=== Rate=$RATE Mode=$MODE (tag=$TAG) ===" >> "$LOG"
...
for rate in 1 2 4 8; do
    run_rate $rate distribution "r${rate}"
done
...
python3 -c "
import json
print(f'{\"Rate\":>5} {\"RealTPOT\":>10} {\"EmuTPOT\":>10} {\"Gap%\":>8} {\"RealConc\":>9} {\"EmuConc\":>9}')
for rate in [1, 2, 4, 8]:
    try:
        r = json.load(open(f'$RESULT_DIR/r{rate}_real_r{rate}.json'))
        e = json.load(open(f'$RESULT_DIR/r{rate}_emu_r{rate}.json'))
        rt = r.get('mean_tpot_ms', 0)
        et = e.get('mean_tpot_ms', 0)
        gap = (et-rt)/rt*100 if rt else 0
        print(f'{rate:>5} {rt:>10.1f} {et:>10.1f} {gap:>+7.1f}% {r.get(\"max_concurrent_requests\",0):>9} {e.get(\"max_concurrent_requests\",0):>9}')
