#!/bin/bash
# M1b (Qwen2.5-3B) on vLLM's canonical ShareGPT file with roofline-
# corrected dense profile. Full 3-stage cycle + BW slope fit + 3-way
# validate.
set -uo pipefail
...
export BENCH_MODEL="Qwen/Qwen2.5-3B-Instruct"
...
if [ ! -d ./results/m1b-canonical-5rate-real ]; then
    ...
    if [ -d ./results/sharegpt-5rate-real ] && [ ! -d ./results/m1b-canonical-5rate-real ]; then
        mv ./results/sharegpt-5rate-real ./results/m1b-canonical-5rate-real
    fi
else
