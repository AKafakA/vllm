#!/bin/bash
# Quick v5 profile validation: 2 rates, 2000 prompts, surrogate on.
# Compare to archive reference (from paper/apr_18/02_v3_baseline_AB.md):
#   r=2  archive TPOT -0.6  TTFT -32.5
#   r=8  archive TPOT -6.0  TTFT -28.5
# If v5 within ±3pp TPOT of archive → v5 validated as overnight baseline.
set -uo pipefail
...
MODEL="Qwen/Qwen3-8B"
PORT=8100
PROFILE="./results/RTX-8000-adaptive-v5-oneround/serving-full.json"
RATES="2 8"
DIR="./results/RTX-8000-v5-quick"
LOG="$DIR/run.log"

mkdir -p "$DIR"
echo "=== v5-quick validation start $(date) ===" > "$LOG"
touch /tmp/vllm_v5_validate.started

if [ ! -f "$PROFILE" ]; then
    echo "FATAL: v5 profile not found" | tee -a "$LOG"
    exit 1
fi
...
echo "=== v5-quick validation DONE $(date) ===" >> "$LOG"
touch "$DIR/.all_done"
touch /tmp/vllm_v5_validate.done
