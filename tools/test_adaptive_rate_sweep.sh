#!/bin/bash
# Rate sweep with ADAPTIVE profile + PROVEN Qwen2.5 setup (2d oracle + DISABLE_DEFER_ADD).
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
source ~/Code/llm/vllm-emulator/.venv/bin/activate
cd ~/Code/llm/vllm-emulator

LOG=/tmp/vllm_adaptive_test.log
echo "=== Adaptive profile rate sweep at $(date) ===" > "$LOG"

MODEL="Qwen/Qwen3-8B"
PORT=8100
NP=200
PROFILE="./results/RTX-8000-adaptive/profiles/serving-Qwen3-8B-adaptive.json"
RESULT_DIR="./results/RTX-8000-adaptive/rate-sweep"
mkdir -p "$RESULT_DIR"
...
run_rate() {
    ...
    python3 -m vllm.entrypoints.cli.main bench serve \
        ...
        --result-filename "${TAG}_real_r${RATE}.json" > /dev/null 2>&1
    cleanup

    # --- EMU with PROVEN setup: 2d oracle + DISABLE_DEFER_ADD ---
    ...
    python3 -m vllm.entrypoints.cli.main bench serve \
        ...
        --percentile-metrics ttft,tpot,itl --metric-percentiles 50,90,99 \
        --save-result --result-dir "$RESULT_DIR" \
        --result-filename "${TAG}_emu_r${RATE}.json" > /dev/null 2>&1
    cleanup

    # Compare
    python3 -c "
import json
r = json.load(open('$RESULT_DIR/${TAG}_real_r${RATE}.json'))
e = json.load(open('$RESULT_DIR/${TAG}_emu_r${RATE}.json'))
...
python3 -c "
import json
print(f'{\"Rate\":>5} {\"RealTPOT\":>10} {\"EmuTPOT\":>10} {\"Gap%\":>8} {\"RealTTFT\":>10} {\"EmuTTFT\":>10} {\"GapT%\":>8} {\"RealConc\":>9} {\"EmuConc\":>9}')
for rate in [1, 2, 4, 8]:
    try:
        r = json.load(open(f'$RESULT_DIR/r{rate}_real_r{rate}.json'))
        e = json.load(open(f'$RESULT_DIR/r{rate}_emu_r{rate}.json'))
        rt = r.get('mean_tpot_ms', 0); et = e.get('mean_tpot_ms', 0)
        rtt = r.get('mean_ttft_ms', 0); ett = e.get('mean_ttft_ms', 0)
        gp = (et-rt)/rt*100 if rt else 0
        gt = (ett-rtt)/rtt*100 if rtt else 0
        print(f'{rate:>5} {rt:>10.1f} {et:>10.1f} {gp:>+7.1f}% {rtt:>10.1f} {ett:>10.1f} {gt:>+7.1f}% {r.get(\"max_concurrent_requests\",0):>9} {e.get(\"max_concurrent_requests\",0):>9}')
