#!/bin/bash
# Test hybrid overhead fix: 3 configs against sweep-warmed baselines
# 1) Split 2D + chain (no Option A) — had great TTFT, bad TPOT
# 2) Option A + chain (1D prefill + 2D decode)
# 3) Option A + pool
set -e
...
run_test() {
    ...
    local RATE=$2
    local TIMER=$3
    echo "  $TAG rate=$RATE timer=$TIMER..."
    cleanup_gpu
    ...
    VLLM_EMULATOR_TIMER_MODE=$TIMER \
...
echo "============================================"
echo "=== Hybrid Fix: 3-Way Comparison ==="
echo "=== $(date) ==="
echo "============================================"

# Config 1: Split 2D + chain (no Option A)
echo ""
echo "=== Config 1: Split 2D + chain ==="
for RATE in 1 4 8; do
    run_test "fix_split2d_chain" $RATE "chain"
done

# Config 2: Split 2D + pool
echo ""
echo "=== Config 2: Split 2D + pool ==="
for RATE in 1 4 8; do
    run_test "fix_split2d_pool" $RATE "pool"
done

# Config 3: Option A + chain
echo ""
echo "=== Config 3: Option A + chain ==="
for RATE in 1 4 8; do
    run_test "fix_optA_chain" $RATE "chain"
done

# Config 4: Option A + pool
echo ""
echo "=== Config 4: Option A + pool ==="
for RATE in 1 4 8; do
    run_test "fix_optA_pool" $RATE "pool"
done
...
echo "============================================"
echo ""
summarize "fix_split2d_chain"
echo ""
summarize "fix_split2d_pool"
echo ""
summarize "fix_optA_chain"
echo ""
summarize "fix_optA_pool"
echo ""
echo "Before fix (split 2D + chain):"
