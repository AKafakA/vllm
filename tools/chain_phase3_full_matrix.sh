#!/bin/bash
# Phase 3 FULL matrix (no budget limit): 4 profiles × 2 workloads × 5 rates
# × 2000 prompts. Per-spec validation; takes ~5 hours total.
set -uo pipefail
export PATH="$HOME/.local/bin:$PATH"
source ~/Code/llm/vllm-emulator/.venv/bin/activate
cd ~/Code/llm/vllm-emulator

MASTER_LOG="/tmp/vllm_apr20_full_matrix.log"
echo "=== full matrix start $(date) ===" > "$MASTER_LOG"
touch /tmp/vllm_apr20_full_matrix.started
...
for PROF_KEY in archive-r2 fixedmix shareptsampled archiver2ext; do
    ...
    for WORKLOAD in random sharegpt; do
        CELL_DIR="./results/workload-emu-${PROF_KEY}-${WORKLOAD}-2000p5r"
        mkdir -p "$CELL_DIR"
        # Real baselines: copy from canonical sources.
...
echo "=== full matrix DONE $(date) ===" >> "$MASTER_LOG"
