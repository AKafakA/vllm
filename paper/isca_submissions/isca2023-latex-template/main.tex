...
%%%%%%%%%%%---SETME-----%%%%%%%%%%%%%
\title{\sysname{}: GPU-Free Emulation of LLM Serving\\via Lightweight Profile-Driven Hooks}
...
This design is distinguished by its \emph{simplicity} and \emph{completeness}:

\begin{enumerate}
\item \textbf{Purely profile-driven.} No CUDA dependency at runtime, no model training, no manual parameter tuning, no extra configuration. A one-time 25-minute profiling pass produces a compact profile ($\sim$27KB) that drives all emulation.

\item \textbf{Minimal implementation.} \sysname{} is implemented as a pure-Python plugin with approximately \todo{X} lines of code. The integration with vLLM requires only a thin hook at the executor boundary---the rest of vLLM runs unmodified.

\item \textbf{Native vLLM experience.} The emulated server exposes the same OpenAI-compatible API and uses the same CLI tools. Switching between real GPU and emulation is a single environment variable. Online serving and offline batch inference both work through the standard vLLM entrypoints.

\item \textbf{Real scheduling and memory management.} Unlike simulators, \sysname{} runs vLLM's \emph{actual} KV cache manager, prefix caching, block allocator, and eviction policies. Researchers studying cache management systems (e.g., LMCache) or scheduling policies can modify vLLM directly and evaluate on CPU-only hosts---without reimplementing any scheduling logic.

\item \textbf{Forward compatibility.} As vLLM adds features---speculative decoding, disaggregated prefill/decode, prefix caching, multi-step scheduling---\sysname{} automatically supports them with no code changes.

\item \textbf{CPU-only operation at scale.} By combining the hook with lightweight CUDA stub libraries and an emulator worker, \sysname{} runs the \emph{same GPU-build binary} on CPU-only hosts, enabling cluster-scale evaluation with tens of emulated instances on commodity hardware.
\end{enumerate}

We evaluate \sysname{} across five GPU configurations (RTX 3060 to A100$\times$4), models from 3B to 72B parameters, and workloads including production traces (ShareGPT~\cite{sharegpt2023}, BurstGPT~\cite{wang2024burstgpt}). For online serving, we achieve \todo{X\%} mean TPOT error and \todo{X\%} mean E2E error across request rates 1--16. For offline batch inference via \texttt{LLM()}, we achieve \todo{X\%} throughput error. We demonstrate tensor-parallel serving (TP=4), prefill/decode disaggregation, and cluster-level evaluation with tens of emulated instances---all on CPU-only CloudLab commodity servers.
...
\textbf{LLM Serving Simulation.} Vidur~\cite{agrawal2024vidur} is a discrete-event simulator for LLM inference that models request flow, scheduling, and GPU execution. It achieves good accuracy for capacity planning but reimplements scheduling logic and KV cache management, creating a fidelity gap with production systems. Changes to vLLM's prefix caching, block allocation, or eviction policies require corresponding simulator updates. \sysname{} avoids this entirely by running the real engine---including real KV cache management, real prefix caching, and real memory pressure handling.
