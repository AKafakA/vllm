...
\paragraph{Profile capture is per-deployment.}
Each (hardware, model, workload) bundle requires a dedicated
capture; a profile captured on one workload shape does not
transfer to a materially different shape
(\S\ref{sec:results-workload}). A dense capture takes
${\approx}2$~hours of GPU time for $r\in\{2,4,8,16,32\}$, versus
days for per-operator databases (Vidur, AIConfigurator, Revati).
The cost is acceptable when amortised over many emulated runs
but remains a non-trivial up-front commitment; finer oracle axes
(per-request prompt length rather than batch-total tokens) could
reduce the shape-match cost, a natural next step.
