...
enabled (vLLM defaults). The \emph{Main} cell is Qwen3-8B on
NVIDIA Quadro RTX~8000 (48\,GB); we then vary one axis at a
time to test emulator generality:
(i)~\emph{Model-scale} substitutes Qwen2.5-3B on the same
RTX~8000, checking that accuracy holds when the forward-pass
cost changes;
(ii)~\emph{Hardware} moves the Main cell to an A10 GPU
(24\,GB, \texttt{max-num-seqs=64} to fit the smaller memory
budget), checking that the pipeline ports across GPU
generations without code change;
(iii)~\emph{Config} disables vLLM's prefix cache
(\texttt{--no-enable-prefix-caching}) on the Main setup,
checking that a non-default engine configuration does not
break the profile-driven oracle.
