...
across three cells at $r\in\{2,4,8,16,32\}$. All three cells use
the same pipeline: density-aware adaptive profile capture with
saturation-weighted rate coverage, and the delivery-ceiling admission
rule at runtime with $\rho^{\star}$ measured from the same profile
pack.}
\label{tab:main}
\small
...
The steady-state metrics (TPOT, ITL, E2E) are the tighter group of
measurements across every cell. TTFT carries more variance because
first-token latency is a mix of admission-path timing (which depends
on the scheduler's own iteration cadence on the real path) and the
first prefill step; the oracle's step-cycle sample captures the
latter, and vLLM's own admission code contributes the former without
any emulator-side proxy.

\subsection{Workload-shape sensitivity}
\label{sec:results-workload}

A profile captured under one input-length distribution cannot
accurately emulate a workload with a materially different shape.
Running a ShareGPT bench against a profile captured under a
fixed-length synthetic workload (256-token prompts, 128-token
outputs) produces regime-dependent errors ranging from $+30\%$ to
$+40\%$ on TTFT at the same $r\in\{2,4,8,16\}$ rates, because the
oracle samples at low-tt buckets inherit different batch-composition
statistics than the bench encounters. This is not a tuning failure;
it is a direct consequence of the shape-match invariant I2. We
quantify it as motivation for per-deployment profiling in
\S\ref{sec:limitations}.

\subsection{Comparison to prior work}
A direct experimental head-to-head with
Vidur~\cite{agrawal2024vidur} is out of scope: Vidur is offline and
event-driven, re-implements the scheduler via
\texttt{sarathi}, and cannot accept the live HTTP workload we
evaluate. Revati~\cite{agrawal2026revati}, from the same group,
identifies Vidur's maintenance burden and proposes CUDA-API
interception as the remedy; their code is not publicly released
and they have not yet demonstrated vLLM v1 support.
AIConfigurator~\cite{nvidia2026aiconfigurator} supports only
datacenter SXM devices (H100/H200/B200/GB200/A100); the
consumer-tier (RTX~8000) and mid-tier (A10) hardware in our
evaluation falls outside its matrix. We defer qualitative
positioning to \S\ref{sec:related}.
...
\paragraph{L1: profile capture is expensive and workload-specific.}
Each (hardware, model, workload) bundle requires a dedicated capture
to reach bucket densities sufficient for reliable oracle sampling. A
profile captured on one workload shape cannot transfer accurately to
a materially different shape (\S\ref{sec:results-workload}); the
methodology therefore requires per-deployment profiling rather than
one-time calibration. For the $r\in\{2,4,8,16,32\}$ range we evaluate,
a dense capture uses approximately $2$ hours of GPU time on our
hardware; amortised over many emulated runs this is reasonable,
but it is a non-trivial up-front cost. Per-operator simulators
(Vidur, AIConfigurator, Revati) require substantially more
profiling time --- on the order of days per (hardware, model)
pair --- to build their operator-level latency databases.

\paragraph{L2: single-node scope.} We validate only single-GPU
single-process deployments. Multi-GPU parallelism (TP, PP, EP) and
cluster-level deployment introduce dynamics the current oracle does
not capture: collective communication latency, NIC bandwidth
sharing, inter-host queue coupling, distributed scheduler state. The
integration point (executor boundary) is per-process by construction;
extending the architecture to multi-GPU requires modelling both the
per-device per-step latency and the distributed coordination term,
which our oracle does not do today.

\paragraph{L3: high-level keying elides low-level variance.} The
(total-tokens, concurrency) oracle is a coarse key for a
phenomenologically dense event. It does not expose
prefix-cache hit/miss effects, KV-cache offloading dynamics under
memory pressure, kernel-launch jitter near peak utilisation, or
speculative-decoding acceptance variance. Each phenomenon has
characterisable behaviour at the measurement level; the current
axis choice hides it. Adding profile axes for these is feasible but
multiplies capture cost super-linearly.

\paragraph{L4: integration bindings track vLLM internals.} While the
integration footprint is small---roughly $400$ lines of plugin code,
no source-level modification of vLLM, and no emulator-side
reimplementation of admission, output processing, or worker-side
bookkeeping---the executor-boundary hook and platform plugin are
pinned to specific vLLM internal APIs. Upstream refactors to those
APIs can break the bindings; our results are reported against
vLLM 0.18.1. This is not a hand-coded mirror of vLLM's behaviour
(the way a scheduler-reimplementation simulator would be); it is
a small surface of integration points that must remain in sync with
upstream evolution.
...
\paragraph{F1: cluster-level emulation.} Extending the oracle with
inter-node round-trip and collective-bandwidth terms, profiled from a
real multi-host measurement. This is the primary direction for
capacity-planning workflows on realistic deployments.

\paragraph{F2: edge-case-aware profile axes.} Per-request prefix-cache
hit rate, KV-offload queue depth, and speculative-decoding acceptance
rate as additional profile dimensions. Selective additions based on
deployment scenario are the likely practical path.

\paragraph{F3: tighter oracle keying at workload boundaries.}
Finer axes such as per-request prompt length (rather than batch-total
tokens) could reduce the shape-match-invariant cost by making the
oracle more robust to partial workload drift between capture and
bench.

\paragraph{F4: upstream-version adaptability.} A small automated test
matrix that exercises the executor-boundary bindings against
multiple vLLM releases would catch API drift early and lower the
maintenance cost of the integration layer.
...
(ii) \emph{maintenance burden} --- no re-implementation of serving
logic; the vLLM v0$\to$v1 architectural transition is absorbed
automatically without code changes on our side, whereas Vidur's
\texttt{sarathi} engine and AIConfigurator's operator database
require manual porting, and Revati's LD\_PRELOAD patches must be
re-synced per vLLM release;
(iii) \emph{profile granularity} --- a single aggregate
step-cycle capture (${\approx}2$~hours per hardware/model pair)
rather than per-operator profiling (days);
(iv) \emph{backend- and hardware-portable by construction} ---
...
(v) \emph{no learned hyperparameters} --- the oracle is empirical
