...
The implementation is correspondingly compact. The
open-source release of \sysname{} is 2.9K lines of Python (plus a 62-line patch to vLLM's gpu_worker.py to wire the hook)
split across four parts: oracle (density-aware step-latency,
713~lines), vLLM integration (platform plugin, executor
hook, GPU cost path, and mocks; 1048~lines (of which 251 is GPU mocking: CUDA API stubs + fake model-runner output)), profile
pipeline (builder, loader, validator; 642~lines), and
orchestration scripts for profile capture and validation
(422~lines). By comparison, the public Python source of
Vidur~\cite{agrawal2024vidur} is ${\approx}11$K lines,
LLM-ServingSim~\cite{cho2024llmservingsim} ${\approx}15$K,
and AIConfigurator~\cite{nvidia2026aiconfigurator} larger
still (its source bytes include pre-collected
hardware-latency tables embedded in Python modules, not
only implementation code). Revati~\cite{agrawal2026revati}
reports ${\approx}800$~lines of C++ for the Timekeeper plus
${\approx}6000$~lines for the Device Emulation Layer, with
${\leq}50$~per-framework patch lines for vLLM and SGLang.
The gap reflects \sysname{}'s re-implementation-free
design: the scheduler, admission path, and output pipeline
are vLLM's own code. \sysname{} is available as an open-source
