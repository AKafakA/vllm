...
\sysname{} differs in four ways that matter in practice.
\emph{(a) Runtime integration}: a vLLM plugin with no source
modification, using the engine's own scheduler and admission
path. \emph{(b) One implementation, two modes}: a
\texttt{realtime} mode for online serving research (live HTTP,
real queueing) and an \texttt{accelerated} time-skip mode for
offline sweeps (analogous to Revati's time warp). Prior systems
offer at most one of these.
\emph{(c) Coarse-grained empirical profile}: a single
step-cycle capture (${\approx}2$ hours per hardware/model)
captures whatever attention kernel vLLM actually runs,
hardware-agnostic by construction. Per-operator approaches
need days of profiling and are tied to the specific kernel
that was profiled.
\emph{(d) No learned model, no hyperparameters}: the oracle
is empirical distribution lookup with CLT-based sample floor;
no training, no feature engineering, no re-training when the
backend changes.
We plan to release \sysname{} as an open-source plugin
concurrent with this publication; all four systems above
require physical access to the target hardware for profile
capture, and the vLLM-plugin form factor inherits vLLM's own
