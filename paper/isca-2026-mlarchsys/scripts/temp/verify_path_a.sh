#!/bin/bash
# Quick verify: Path A (regular GPU emulator) still <5% after all changes
set -e
...
python3 -m vllm.entrypoints.openai.api_server \
    ...
    > /workspace/verify_a_real.log 2>&1 &
...
for rate in 1 2 4; do
    python3 -m vllm.entrypoints.cli.main bench serve \
        ...
        --result-filename "verify_a_real_rate${rate}.json" > /dev/null 2>&1
done
pkill -f api_server 2>/dev/null || true; sleep 2; pkill -9 -f EngineCore 2>/dev/null || true; sleep 5

# GPU emulator (Path A: NO MOCK_CUDA, regular serving profile + executor hook)
echo "=== GPU Emu (Path A) ==="
...
python3 -m vllm.entrypoints.openai.api_server \
    --model "${MODEL}" --max-model-len 4096 --port ${PORT} --trust-remote-code \
    > /workspace/verify_a_emu.log 2>&1 &
for i in $(seq 1 120); do
    if curl -s http://localhost:${PORT}/health > /dev/null 2>&1; then break; fi; sleep 1
done
...
for rate in 1 2 4; do
    python3 -m vllm.entrypoints.cli.main bench serve \
        ...
        --percentile-metrics ttft,tpot --metric-percentiles 50,99 \
        --save-result --result-dir "${RESULT_DIR}/online" \
        --result-filename "verify_a_emu_rate${rate}.json" > /dev/null 2>&1
done
...
for rate in [1, 2, 4]:
    rf = f'{rd}/verify_a_real_rate{rate}.json'
    ef = f'{rd}/verify_a_emu_rate{rate}.json'
