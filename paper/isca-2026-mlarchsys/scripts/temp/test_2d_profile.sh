#!/bin/bash
# Build 2D profile (prefill/decode separated) and test
set -e
...
python3 -m vllm.entrypoints.openai.api_server \
    ...
    > /workspace/2d_real_server.log 2>&1 &
...
for rate in 1 2 4; do
    ...
    python3 -m vllm.entrypoints.cli.main bench serve --model "${MODEL}" --base-url http://localhost:${PORT} \
        ...
        --save-result --result-dir "${RESULT_DIR}/online" --result-filename "2d_real_rate${rate}.json" 2>&1 | grep -E "TTFT|TPOT"
done
pkill -f api_server 2>/dev/null || true; sleep 2; pkill -9 -f EngineCore 2>/dev/null || true; sleep 5

# Emulator with 2D profile
echo ""
echo "Emulator (2D profile)..."
...
python3 -m vllm.entrypoints.openai.api_server \
    --model "${MODEL}" --max-model-len 4096 --port ${PORT} --trust-remote-code \
    > /workspace/2d_emu_server.log 2>&1 &
for i in $(seq 1 120); do
    if curl -s http://localhost:${PORT}/health > /dev/null 2>&1; then break; fi; sleep 1
done
grep ExecutorEmulatorHook /workspace/2d_emu_server.log | head -1
for rate in 1 2 4; do
    echo "  Emu rate=${rate}..."
    python3 -m vllm.entrypoints.cli.main bench serve --model "${MODEL}" --base-url http://localhost:${PORT} \
        --dataset-name random --random-input-len 256 --random-output-len 128 \
        --num-prompts 50 --request-rate ${rate} --percentile-metrics ttft,tpot --metric-percentiles 50,99 \
        --save-result --result-dir "${RESULT_DIR}/online" --result-filename "2d_emu_rate${rate}.json" 2>&1 | grep -E "TTFT|TPOT"
done
...
for rate in [1, 2, 4]:
    rf = f'{rd}/2d_real_rate{rate}.json'
    ef = f'{rd}/2d_emu_rate{rate}.json'
