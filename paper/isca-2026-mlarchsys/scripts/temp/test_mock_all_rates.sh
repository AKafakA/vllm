#!/bin/bash
# Test CUDA mock path at all rates on Vast GPU host
set -e
...
echo "=== Mock path on GPU host — all rates ==="
...
python3 -m vllm.entrypoints.openai.api_server \
    ...
    > /workspace/mock_allrates_server.log 2>&1 &
...
if ! curl -s http://localhost:${PORT}/health > /dev/null 2>&1; then
    echo "  FAILED"
    tail -20 /workspace/mock_allrates_server.log
    exit 1
fi
...
for rate in 1 2 4; do
    ...
    python3 -m vllm.entrypoints.cli.main bench serve \
        ...
        --result-filename "mock_allrates_rate${rate}.json" 2>&1 | grep -E "TTFT|TPOT"
done
...
for rate in [1, 2, 4]:
    ...
    mf = f'{rd}/mock_allrates_rate{rate}.json'
