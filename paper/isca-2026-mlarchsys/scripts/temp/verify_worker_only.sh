#!/bin/bash
# Verify: worker hook ONLY (NO executor hook) — this was the proven <3% path
set -e
...
python3 -m vllm.entrypoints.openai.api_server \
    ...
    > /workspace/worker_only_real.log 2>&1 &
...
for rate in 1 2 4; do
    python3 -m vllm.entrypoints.cli.main bench serve --model "${MODEL}" --base-url http://localhost:${PORT} \
        ...
        --save-result --result-dir "${RESULT_DIR}/online" --result-filename "wonly_real_rate${rate}.json" > /dev/null 2>&1
done
pkill -f api_server 2>/dev/null || true; sleep 2; pkill -9 -f EngineCore 2>/dev/null || true; sleep 5

# Worker hook ONLY — use serving profile but NO EXECUTOR_HOOK
echo "=== Emu (worker hook only, NO executor hook) ==="
VLLM_EMULATOR_ENABLE_ORACLE=1 \
VLLM_EMULATOR_PROFILE_PACK="${RESULT_DIR}/profiles/serving-1.5b-tp1-step-cycle.json" \
VLLM_EMULATOR_MODE=realtime \
python3 -m vllm.entrypoints.openai.api_server \
    --model "${MODEL}" --max-model-len 4096 --port ${PORT} --trust-remote-code \
    > /workspace/worker_only_emu.log 2>&1 &
for i in $(seq 1 120); do
    if curl -s http://localhost:${PORT}/health > /dev/null 2>&1; then break; fi; sleep 1
done
for rate in 1 2 4; do
    python3 -m vllm.entrypoints.cli.main bench serve --model "${MODEL}" --base-url http://localhost:${PORT} \
        --dataset-name random --random-input-len 256 --random-output-len 128 \
        --num-prompts 30 --request-rate ${rate} --percentile-metrics ttft,tpot --metric-percentiles 50,99 \
        --save-result --result-dir "${RESULT_DIR}/online" --result-filename "wonly_emu_rate${rate}.json" > /dev/null 2>&1
done
...
for rate in [1, 2, 4]:
    rf = f'{rd}/wonly_real_rate{rate}.json'
    ef = f'{rd}/wonly_emu_rate{rate}.json'
