#!/bin/bash
# Test with warmup prompts to eliminate cold CUDA graph effect
set -e
...
# Real baseline with warmup
echo "=== Real (with ${WARMUP} warmup prompts) ==="
python3 -m vllm.entrypoints.openai.api_server \
    --model "${MODEL}" --max-model-len 4096 --port ${PORT} --trust-remote-code \
    > /workspace/warmup_real.log 2>&1 &
...
for rate in 1 2 4; do
    ...
    python3 -m vllm.entrypoints.cli.main bench serve --model "${MODEL}" --base-url http://localhost:${PORT} \
        ...
        --num-prompts 50 --request-rate ${rate} --num-warmups ${WARMUP} \
        --percentile-metrics ttft,tpot --metric-percentiles 50,99 \
        --save-result --result-dir "${RESULT_DIR}/online" --result-filename "warmup_real_rate${rate}.json" 2>&1 | grep -E "TTFT|TPOT"
done
pkill -f api_server 2>/dev/null || true; sleep 2; pkill -9 -f EngineCore 2>/dev/null || true; sleep 5

# Emulator with warmup + prefill overhead
echo ""
echo "=== Emu (2D profile + PFOH=5000 + warmup) ==="
...
python3 -m vllm.entrypoints.openai.api_server \
    --model "${MODEL}" --max-model-len 4096 --port ${PORT} --trust-remote-code \
    > /workspace/warmup_emu.log 2>&1 &
for i in $(seq 1 120); do
    if curl -s http://localhost:${PORT}/health > /dev/null 2>&1; then break; fi; sleep 1
done
grep ExecutorEmulatorHook /workspace/warmup_emu.log | head -1
for rate in 1 2 4; do
    echo "  Emu rate=${rate}..."
    python3 -m vllm.entrypoints.cli.main bench serve --model "${MODEL}" --base-url http://localhost:${PORT} \
        --dataset-name random --random-input-len 256 --random-output-len 128 \
        --num-prompts 50 --request-rate ${rate} --num-warmups ${WARMUP} \
        --percentile-metrics ttft,tpot --metric-percentiles 50,99 \
        --save-result --result-dir "${RESULT_DIR}/online" --result-filename "warmup_emu_rate${rate}.json" 2>&1 | grep -E "TTFT|TPOT"
done
...
for rate in [1, 2, 4]:
    rf = f'{rd}/warmup_real_rate{rate}.json'
    ef = f'{rd}/warmup_emu_rate{rate}.json'
