#!/bin/bash
# Test prefill-only overhead to fix TTFT without hurting TPOT
set -e
...
# Real baseline (reuse from oh_test if fresh enough)
echo "=== Real baseline ==="
python3 -m vllm.entrypoints.openai.api_server \
    --model "${MODEL}" --max-model-len 4096 --port ${PORT} --trust-remote-code \
    > /workspace/pfoh_real.log 2>&1 &
...
for rate in 1 2 4; do
    python3 -m vllm.entrypoints.cli.main bench serve --model "${MODEL}" --base-url http://localhost:${PORT} \
        ...
        --save-result --result-dir "${RESULT_DIR}/online" --result-filename "pfoh_real_rate${rate}.json" > /dev/null 2>&1
done
pkill -f api_server 2>/dev/null || true; sleep 2; pkill -9 -f EngineCore 2>/dev/null || true; sleep 5

for PFOH in 5000 10000 20000; do
    echo "=== Emu (prefill_overhead=${PFOH}us) ==="
    VLLM_EMULATOR_ENABLE_ORACLE=1 \
    VLLM_EMULATOR_PROFILE_PACK="${PROFILE}" \
    VLLM_EMULATOR_MODE=realtime \
    VLLM_EMULATOR_EXECUTOR_HOOK=1 \
    VLLM_EMULATOR_PREFILL_OVERHEAD_US=${PFOH} \
    python3 -m vllm.entrypoints.openai.api_server \
        --model "${MODEL}" --max-model-len 4096 --port ${PORT} --trust-remote-code \
        > /workspace/pfoh_emu_${PFOH}.log 2>&1 &
    for i in $(seq 1 120); do
        if curl -s http://localhost:${PORT}/health > /dev/null 2>&1; then break; fi; sleep 1
    done
    for rate in 1 2 4; do
        python3 -m vllm.entrypoints.cli.main bench serve --model "${MODEL}" --base-url http://localhost:${PORT} \
            --dataset-name random --random-input-len 256 --random-output-len 128 \
            --num-prompts 50 --request-rate ${rate} --percentile-metrics ttft,tpot --metric-percentiles 50,99 \
            --save-result --result-dir "${RESULT_DIR}/online" --result-filename "pfoh_emu_${PFOH}_rate${rate}.json" > /dev/null 2>&1
    done
    pkill -f api_server 2>/dev/null || true; sleep 2; pkill -9 -f EngineCore 2>/dev/null || true; sleep 5
done
...
print(f'{\"PFOH\":>6} {\"Rate\":>4} {\"TTFT_err\":>9} {\"TPOT_err\":>9}')
for pfoh in [5000, 10000, 20000]:
    for rate in [1, 2, 4]:
        rf = f'{rd}/pfoh_real_rate{rate}.json'
        ef = f'{rd}/pfoh_emu_{pfoh}_rate{rate}.json'
        if os.path.exists(rf) and os.path.exists(ef):
            ...
            print(f'{pfoh:>6} {rate:>4} {te:>+8.1f}%{tok} {pe:>+8.1f}%{pok}')
    print()
"
echo "DONE"
